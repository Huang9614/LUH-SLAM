...
class Particle:
    ...
    def H_Ql_jacobian_and_measurement_covariance_for_landmark(
        ...
        # --->>> Insert your previous code here.
        H = self.dh_dlandmark(self.pose,self.landmark_positions[landmark_number],\
                              scanner_displacement)
        HT = H.T
        sigma_K = self.landmark_covariances[landmark_number]

        Ql = np.dot(H,np.dot(sigma_K,HT)) + Qt_measurement_covariance
        return (H, Ql)
    ...
    def wl_likelihood_of_correspondence(self, measurement,
        ...
        # --->>> Insert your previous code here.
        z = measurement
        exp_z = self.h_expected_measurement_for_landmark(landmark_number,scanner_displacement)
        delta_z = z - exp_z
        delta_zT = delta_z.T
        ...
        q = -np.dot(delta_zT,np.dot(np.linalg.inv(Ql),delta_z))/2
        
        l = exp(q)/(2*pi*sqrt(detQl))
    ...
    def initialize_new_landmark(self, measurement_in_scanner_system,
        ...
        landmark_cov = np.dot(H_inv,np.dot(Qt_measurement_covariance,H_inv.T))
        self.landmark_positions.append(m)  # Replace this.
        self.landmark_covariances.append(landmark_cov)  # Replace this.

    def update_landmark(self, landmark_number, measurement,
                        Qt_measurement_covariance, scanner_displacement):
        """Update a landmark's estimated position and covariance."""
        # --->>> Insert your previous code here.
        mu_old = self.landmark_positions[landmark_number]
        ...
        self.landmark_positions[landmark_number] = mu_old + np.dot(K,delta_z)
    ...
    def update_particle(self, measurement, measurement_in_scanner_system,
        ...
        number_of_landmarks,Qt_measurement_covariance,scanner_displacement) 
                            # --->>> Replace this by
        ...
        # Else update the particle's EKF for the corresponding particle.
            ...
            # Replace this by a search for max, argmax.
            # Add code to update_landmark().
            w = max(likelihoods)
            index = np.argmax(likelihoods) # Replace this by a search for max, argmax.
            self.update_landmark(index,measurement,Qt_measurement_covariance,\
                                 scanner_displacement)
            return w
