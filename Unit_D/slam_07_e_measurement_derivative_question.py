...
class ExtendedKalmanFilter:
    ...
    def dh_dstate(state, landmark, scanner_displacement):
        ...
        q=(landmark[0]-xl)**2+(landmark[1]-yl)**2
        delta_x = landmark[0]-xl
        delta_y = landmark[1]-yl
        
        hx1 = -delta_x/sqrt(q)
        hy1 = -delta_y/sqrt(q)
        ht1 =  d*(delta_x*sin(theta)-delta_y*cos(theta))/sqrt(q)

        hx2 = delta_y/q
        hy2 = -delta_x/q
        ht2 = (-d*(delta_x*cos(theta)+delta_y*sin(theta))/q)-1    

        return array([[hx1,hy1,ht1], [hx2, hy2, ht2]]) # Replace this.
